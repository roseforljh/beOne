...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .then((user) => {
        setAuth(token, user);
        toast.success(`Signed in${provider ? ` via ${provider}` : ''}`);
  ...
  const goOAuth = (provider: 'github' | 'google') => {
      ...
      <div className="w-full max-w-[350px] space-y-6">
        ...
        <div className="grid gap-6">
          <form onSubmit={(e) => { e.preventDefault(); handleDevLogin(); }}>
            <div className="grid gap-4">
              ...
              <Button disabled={loading} className="w-full">
                {loading ? 'Signing in...' : 'Sign In with Dev Account'}
              </Button>
            </div>
          </form>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button variant="outline" type="button" onClick={() => goOAuth('github')} disabled={loading}>
              <Github className="mr-2 h-4 w-4" />
              Github
            </Button>
            <Button variant="outline" type="button" onClick={() => goOAuth('google')} disabled={loading}>
